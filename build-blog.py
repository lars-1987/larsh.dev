...
HEAD_CSS = """
:root{
  --bg:#F2ECDF;--bg-warm:#EAE2D0;--bg-elevated:#E7DFCD;--bg-subtle:#DED5C1;
  --sage:#E7C9A8;--sage-mid:#C58455;--fg:#16140F;--muted:#6E655A;--muted-strong:#3f382f;
  --border:rgba(22,20,15,.16);--border-strong:rgba(22,20,15,.30);
  --accent:#9C5A28;--accent-dim:rgba(197,132,85,.20);
  --shadow-sm:0 10px 24px -14px rgba(22,20,15,.30);
  --shadow-lg:0 40px 80px -34px rgba(22,20,15,.40);
...
body{background:var(--bg);color:var(--fg);font-family:var(--font);font-size:var(--fs-base);
  line-height:1.65;-webkit-font-smoothing:antialiased;overflow-x:hidden;padding-top:64px}
body::before{content:"";position:fixed;inset:0;z-index:0;pointer-events:none;background:
  radial-gradient(1200px 620px at 78% -12%,rgba(227,154,99,.14),transparent 60%),
  radial-gradient(1000px 520px at -5% 102%,rgba(197,132,85,.12),transparent 55%)}
a{color:inherit;text-decoration:none}
.snav{position:fixed;left:50%;top:16px;transform:translateX(-50%);z-index:60;display:flex;align-items:center;gap:.16rem;padding:.5rem;border-radius:999px;background:rgba(255,251,244,.9);backdrop-filter:blur(14px) saturate(1.2);border:1px solid var(--border);box-shadow:var(--shadow-sm)}
.snav__links{display:flex;align-items:center;gap:.16rem}
.snav a{font-family:var(--font);font-weight:600;font-size:clamp(.79rem,1.32vw,.95rem);color:var(--fg);padding:.62em 1.05em;border-radius:999px;white-space:nowrap;transition:color .3s,background .3s}
.snav a:hover{background:rgba(22,20,15,.07)}
.snav a.is-active{background:#E39A63;color:#16140F}
.snav__burger{display:none}
.smenu{position:fixed;inset:0;z-index:70;background:#0E0D0C;color:#F3EEE3;display:flex;flex-direction:column;justify-content:center;gap:1rem;padding:6vw;transform:translateY(-100%);transition:transform .5s var(--ease);visibility:hidden}
html.menu-open .smenu{transform:none;visibility:visible}
.smenu a{font-family:var(--font);font-weight:800;text-transform:uppercase;font-size:clamp(2rem,9vw,3.4rem);letter-spacing:-.02em;color:#F3EEE3}
.smenu a:hover{color:#E39A63}
@media(max-width:680px){.snav .snav__links{display:none}.snav{gap:0;padding:.35rem}.snav__burger{display:inline-flex;align-items:center;gap:.55em;font-family:var(--mono);font-size:.72rem;letter-spacing:.1em;text-transform:uppercase;color:var(--fg);padding:.6em 1em;background:none;border:0;cursor:pointer}.snav__burger b{display:block;width:20px;height:2px;background:var(--fg);position:relative}.snav__burger b::before,.snav__burger b::after{content:"";position:absolute;left:0;width:20px;height:2px;background:var(--fg)}.snav__burger b::before{top:-6px}.snav__burger b::after{top:6px}}
...
/* prose */
.prose{min-width:0}
/* cap flowing text for readability, but let tables + code blocks use the full width */
.prose>p,.prose>ul,.prose>ol,.prose>blockquote,.prose>h2,.prose>h3{max-width:70ch}
...
.prose table{width:100%;min-width:720px;border-collapse:collapse;font-size:.85rem;line-height:1.45}
.prose thead th{text-align:left;vertical-align:bottom;font-weight:600;color:var(--fg);
  background:var(--bg-elevated);padding:.7em .85em;border-bottom:1px solid var(--border-strong)}
.prose tbody td{vertical-align:top;padding:.62em .85em;color:var(--muted-strong);
  border-bottom:1px solid var(--border)}
.prose tbody tr:last-child td{border-bottom:0}
.prose tbody tr:nth-child(even){background:var(--bg-warm)}
.prose th:first-child,.prose td:first-child{color:var(--fg);font-weight:600;white-space:nowrap}
...
NAV_HTML = """<nav class="snav" aria-label="Primary">
  ...
  <button class="snav__burger" id="snav-burger" aria-label="Open menu" aria-expanded="false"><b></b>Menu</button>
</nav>
...
NAV_SCRIPT = """<script>
(function(){
  var b=document.getElementById('snav-burger'); if(!b) return;
  var set=function(o){document.documentElement.classList.toggle('menu-open',o);b.setAttribute('aria-expanded',o?'true':'false');};
  b.addEventListener('click',function(){set(!document.documentElement.classList.contains('menu-open'));});
  document.querySelectorAll('#smenu a').forEach(function(a){a.addEventListener('click',function(){set(false);});});
...
<link rel="preload" href="../vendor/fonts/archivo-100-900-latin.woff2" as="font" type="font/woff2" crossorigin />
...
<script type="application/ld+json">{json.dumps(blog_ld, ensure_ascii=False)}</script>'''
    ...
    urls = [(SITE + '/', None, '1.0'), (SITE + '/resume/', None, '0.8'), (SITE + '/blog/', None, '0.8')]
